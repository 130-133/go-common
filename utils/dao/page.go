...
func (page *Page) SetFields(fields []string) {
	page.fields = fields
}

func (page *Page) GetFields() []string {
	return page.fields
}

func (page *Page) GetOption() *Option {
	return page.opt.init()
}
...
func (page *Page) GetBsonSort() bson.D {
	sort := bson.D{}
	for _, val := range page.orders {
		asc := -1
		if val.asc {
		...
		sort = append(sort, bson.E{Key: val.column, Value: asc})
	}
	return sort
}
