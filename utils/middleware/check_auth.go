...
func (a AuthKeys) GetCheckAuthFun(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if hAuthorization != "" {
			...
			uin, err = postAuthFun(a.AuthSecret, help.GetRawBody(r))
		}
		if err != nil {
			data := errorx.NewSystemError("登录状态已失效、请重新登录", 0).(*errorx.MiniCodeError).Data()
...
func authorizationFun(authKeys AuthKeys, authorization string) (uin string, err error) {
	...
	case "Basic":
		// 四小强 第二形态（旧）
		var hAuthorizationByte []byte
		hAuthorizationByte, err = base64.StdEncoding.DecodeString(authToken)
		if err != nil {
			return
		}
		...
		s2t, _ := strconv.ParseInt(s2tStr, 10, 64)
		...
		if err = fourAuthCheck(authKeys.AuthSecret, uin, time, s2t, sign); err != nil {
			return
		}
	default:
		err = errors.New("鉴权失败")
